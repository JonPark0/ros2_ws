...
class StorageLedger:
    ...
    def _safe_distance(self, a: Optional[int], b: int) -> float:
        """Approximate driven distance between two stations for runtime
        pick-order decisions. Prefers the Bezier-curve estimate (matches
        planning-time route ordering); falls back to Euclidean, then a
        station-id difference if no calculator is available at all."""
        if a is None or self._calc is None:
            return float(abs(int(b)))
        try:
            return float(self._calc.bezier_station_to_station(int(a), int(b)))
